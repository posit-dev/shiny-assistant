...
from shiny.types import Jsonifiable as JSONifiable
...
class Toolbox:
    def __init__(self, *tools: WrappedTool[P, ChatCompletionToolMessageParam]):
        for tool in tools:
...
def func_to_schema(
    func: Callable[..., object], name: str | None = None
) -> dict[str, object]:
...
def type_to_json_schema(t: type, desc: str | None = None) -> dict[str, JSONifiable]:
    origin = get_origin(t)
    args = get_args(t)
...
def type_dict(
    type: str, description: str | None, **kwargs: JSONifiable
